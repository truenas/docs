...
* Configure **Syslog** settings to send logs to an external server ([CORE](https://www.truenas.com/docs/core/13.0/uireference/system/advanced/) | [SCALE](https://www.truenas.com/docs/scale/systemsettings/)).
* In TrueNAS 24.04 (Dragonfish) or later, locally [monitor and review audit logs](https://www.truenas.com/docs/scale/systemsettings/audit/) using the **Audit** screen.
...
Use complex passwords and [Two-Factor Authentication](https://www.truenas.com/docs/scale/credentials/users/) for all TrueNAS administrator accounts.
...
Follow the [iSCSI creation wizard](https://www.truenas.com/docs/scale/shares/iscsi/) to create an iSCSI share.
...
During [NFS share creation](https://www.truenas.com/docs/scale/shares/nfs/addingnfsshares/), define which systems are authorized for share connections.
...
Select a **Purpose** during [SMB share creation](https://www.truenas.com/docs/scale/shares/smb/).
...
[SSH settings](https://www.truenas.com/docs/scale/systemsettings/services/sshservice/) are in **Services** after clicking the <span class="iconify" data-icon="mdi:pencil"></span> (pencil).
...
If a critical situation requires root login, first set up two-factor authentication ([CORE](https://www.truenas.com/docs/core/13.0/coretutorials/systemconfiguration/usingtwofactorauthentication/) | [SCALE](https://www.truenas.com/docs/scale/credentials/users/)) as an extra layer of security.
...
After [configuring a VM](https://www.truenas.com/docs/scale/apps/), disable any VNC or SPICE virtual machine display devices.
