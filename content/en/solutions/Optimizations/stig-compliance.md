...
</file>. Refer to documentation on upgrading existing systems found [here]({{< ref "/software-status" >}}).

If side-grading (migrating) from an earlier release to the latest TrueNAS release, follow guidance in the [Preparing to Migrate](https://www.truenas.com/docs/scale/gettingstarted/migrate/migrateprep/) article before beginning the migration.

If clean-installing TrueNAS for the first time, follow guidance in the [Installation Instructions](https://www.truenas.com/docs/scale/gettingstarted/install/) articles.

### Prerequisites

If you are installing TrueNAS on servers other than licensed TrueNAS Enterprise systems, you should read and follow guidance in the [TrueNAS Hardware Guide](https://www.truenas.com/docs/scale/gettingstarted/).

## Configuring TrueNAS

After installing TrueNAS, users must configure network, storage, users, sharing, and data backup solutions. See [Configuration Instructions](https://www.truenas.com/docs/scale/gettingstarted/configure/) for more information.
...
See [Using Administrator Logins](https://www.truenas.com/docs/scale/credentials/adminroles/) for more information on administration user accounts.
...
See [SSH Service](https://www.truenas.com/docs/scale/systemsettings/services/sshservice/) for more information.
...
See [Managing Users](https://www.truenas.com/docs/scale/credentials/users/manageusers/) and [Using Administrator Logins](https://www.truenas.com/docs/scale/credentials/adminroles/) for more information on creating users and allowing `sudo` command access.
...
See [Managing Global Two-Factor Authentication](https://www.truenas.com/docs/scale/credentials/users/) for more information
...
See [Audit Logs](https://www.truenas.com/docs/scale/systemsettings/audit/) for more information on TrueNAS audit logs and configuring auditing settings.
...
Configure TrueNAS to send system logs to an external server using the **Syslog** widget on the **System > Advanced Settings** screen in [release 24.10 and later](https://www.truenas.com/docs/scale/systemsettings/).
...
| **Certificates** | <ul><li>**Certificate is Expiring Soon**</li><li>**Certificate is Expiring**</li><li>**Certificate has Expired**</li></ul> | Set alert level preferences to send notifications when certificates near expiration. This allows you to renew or replace certificates before they affect certificate-dependent functions. |
...
| **Sharing** | <ul><li>**Deprecated Service Configuration Detected**</li><li>**Deprecated Service is Running**</li></ul> | Set alert preferences to notify you when the system detects deprecated services or configurations. Consult release notes and tutorials for information on affected Share protocols and their replacements. |
|  | <ul><li>**IP Addresses Bound to an iSCSI Portal Were Not Found**</li><li>**NFS Services Could Not Bind to Specific IP Addresses Using 0.0.0**</li><li>**NFS shares reference hosts that could not be resolved**</li></ul> | Set alert preferences to notify you of failed or missing network connections so you can update configurations promptly. |
...
To increase network, consider configuring allowed domains on the [**Network > Global Configuration** screen](https://www.truenas.com/docs/scale/systemsettings/).

To increase share security, configure allowed host names or IP addresses for [SMB shares](https://www.truenas.com/docs/scale/shares/smb/addmanagesmbshares/#configuring-share-advanced-options-settings) or [NFS shares](https://www.truenas.com/docs/scale/shares/nfs/addingnfsshares/#adding-nfs-share-networks-and-hosts).
...
See [Setting Up Permissions](https://www.truenas.com/docs/scale/datasets/) for more information.
...
See [Windows Shares](https://www.truenas.com/docs/scale/shares/smb/) for more information on SMB shares, creating shares, and configuring settings including permissions.

NFS shares do not have the same setting options and rely on the dataset ACL settings.
See [NFS Shares](https://www.truenas.com/docs/scale/shares/nfs/addingnfsshares/) for more information.
...
See [Storage Encryption](https://www.truenas.com/docs/scale/datasets/encryption/) for more information on encrypting datasets.
...
See [Encrypting Cloud Sync Tasks](https://www.truenas.com/docs/scale/dataprotection/cloudsynctasks/) for more information on encrypting cloud sync tasks, or [Adding Transfer Encryption](https://www.truenas.com/docs/scale/dataprotection/replication/) for information on adding encryption to remote replication tasks.
...
See [Boot Pool Management](https://www.truenas.com/docs/scale/systemsettings/) for more information on working with boot pool environments.
