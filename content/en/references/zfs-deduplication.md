...
[Special vdev](https://www.truenas.com/docs/scale/storage/pools/) SSDs receive continuous, heavy I/O.
...
* **Cause**: When ZFS has fast, special vdev SSD disks, sufficient RAM, and is not limited by disk I/O, then the hash calculation becomes the next bottleneck.
 ...
 This is often encountered with [pool scrubs](https://www.truenas.com/docs/scale/storage/pools/managepools/) and it can be necessary to pause the scrub temporarily when other tasks are a priority.
